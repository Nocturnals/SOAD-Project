...
class docMatch extends React.Component {
  ...
  handleSubmit(e) {
    ...
    const { text1, text2, APIKEY } = this.state;
    console.log(text1 + text2 + APIKEY);
    if (text1 && text2 && APIKEY) {
      this.props.matching(text1, text2, APIKEY);
    }
  }
  displayText = plager => {
    let marks = [];
    ...
    if (this.props.docMatch.isLoading === false) {
      if (plager) {
        ...
        if (this.props.docMatch.vPlag.valueOne < 1) {
          marks.push(
            <h3>
              verbatimSimilarity ({this.props.docMatch.vPlag.valueOne * 100}%)
            </h3>
          );
        } else {
          marks.push(<h3>verbatimSimilarity ({100}%)</h3>);
        }
      } else {
        ...
        if (this.props.docMatch.pPlag.valueOne < 1) {
          marks.push(
            <h3>
              ...
              {this.props.docMatch.pPlag.valueOne * 100}
              %)
            </h3>
          );
        } else {
          marks.push(<h3>paraphrasing Similarity ({100}%)</h3>);
        }
      }
